...
class A02 {
	void input(){
		...
		print(kind, output_degree, output_degree2);
	...
	void print(String kind, double output_degree, double output_degree2){
		if(kind.equalsIgnoreCase("C"))	//입력된 온도가 섭씨온도로 지정하면 화씨온도로 변환
		{	
			System.out.print("변환된 온도는 " + output_degree2 + " 입니다.");		
		}
		else if(kind.equalsIgnoreCase("F"))  //입력된 온도가 화씨온도로 지정하면 섭씨온도로 변환
		{	
